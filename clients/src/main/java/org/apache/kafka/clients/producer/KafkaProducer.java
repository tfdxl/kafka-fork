...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    private static int configureRetries(ProducerConfig config, boolean idempotenceEnabled, Logger log) {
        ...
        //配置了幂等，但是不允许重试，那就是配置有问题，不能重试还要幂等，你脑子有问题吗
    ...
    /**
     * 异步的发送消息到topic并且调用callback，当send被broker ack的时候
     * Asynchronously send a record to a topic and invoke the provided callback when the send has been acknowledged.
     * <p>
     * 发送是异步的并且一旦消息存储到待发送的buffer区内，方法就会立即返回，允许并行不阻塞的发送多个消息
    ...
    private ClusterResourceListeners configureClusterResourceListeners(Serializer<K> keySerializer, Serializer<V> valueSerializer, List<?>... candidateLists) {
        final ClusterResourceListeners clusterResourceListeners = new ClusterResourceListeners();
        for (final List<?> candidateList : candidateLists) {
