...
/**
  ...
  *
  * @param file         The index file 磁盘的索引文件
...
abstract class AbstractIndex[K, V](@volatile var file: File, val baseOffset: Long,
  ...
  protected var mmap: MappedByteBuffer = {
    //如果文件不存在，那么创建新的文件并返回true,否则返回false
    ...
    try {
      ...
      if (newlyCreated) {
        ...
        //根据maxIndexSize的值对索引文件进行扩容，结果就是小于maxIndexSize的最大的8的倍数
      ...
      if (newlyCreated)
      //将创建的索引文件的position设置为0，从头开始写文件
        idx.position(0)
      else
      //对于原来就存在的索引文件，将position移动到所有索引项的结束位置，防止数据覆盖
